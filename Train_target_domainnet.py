...
def train(epoch, net, moco_model, optimizer, trainloader, banks):
    ...
    for batch_idx, batch in enumerate(trainloader): 
        ...
        l = loss_cls + loss_ctr + 0.01*loss_div
...
optimizer = optim.SGD(net.parameters(), lr=args.lr, weight_decay=5e-4, momentum=0.9, nesterov=True)

net = nn.DataParallel(net)
momentum_net = nn.DataParallel(momentum_net)

moco_model = AdaMoCo(src_model = net, momentum_model = momentum_net, features_length=net.module.bottleneck_dim, num_classes=args.num_class, dataset_length=len(train_dataset), temporal_length=args.temporal_length)
#moco_model = AdaMoCo(src_model = net, momentum_model = momentum_net, features_length=net.bottleneck_dim, num_classes=args.num_class, dataset_length=len(train_dataset))
